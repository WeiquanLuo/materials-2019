...
To appreciate how far Frodo and Sam had to travel, include a scale with `annotation_scale`.
Also show North in a more map typical way with `annotation_north_arrow`
...
Get these matrices into a single data frame `ozplus` with variables `long`, `lat`, `group`, `order`  and plot with the command:
...
- Read the shapefile (there are usually shapefiles at different administrative levels - pick one). Thin the shapefile (or you might crash your R session; shapefiles can be big), then export the polygons and plot in `ggplot2`.
