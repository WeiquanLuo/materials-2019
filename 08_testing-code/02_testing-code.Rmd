...
Rstudio integrates with `testthat` and makes it easy to run your tests after each change in the code.

```
> usethis::use_testthat()
✔ Setting active project to './test_projects/helloR'
✔ Adding 'testthat' to Suggests field in DESCRIPTION
✔ Creating 'tests/testthat/'
✔ Writing 'tests/testthat.R'
```

Test files go in tests/testthat/test_<some-description>.R

To run your tests, Ctrl-Shift-T or use the build menu
...
The `testthis` package provides some RStudio addins to help with testing.

```{r, eval = F}
install.packages("testthis")
```

Create a new test file for the currently selected file by running `testthis::test_skeleton()` in the console.

```
> testthis::test_skeleton()
* Creating `tests/testthat/test_hello.R`
```

.center[![:scale 90%](testthis_skeleton_file.png)]

---
class:inverse
## Your Turn

1. Create a new R package, `helloR`

2. Set up the package to work with `testthat`

3. Create a test skeleton file with `testthis::test_skeleton()` for the default `hello.R` file in the R package subdirectory

4. Add your package to github with `usethis::use_github()`
...
Add an R function to the `hello.R` file.
...
1. Write the tests for this function in the appropriate place in `tests/testthat/test_hello.R`

2. Write the function in `R/hello.R`. Upload `R/hello.R` to https://gist.github.com/ (you only need to upload the single file).
.small[Don't forget to add assertions to your function to check inputs!]

3. Use your tests on your neighbor's function using the link to their github gist - do they pass?
